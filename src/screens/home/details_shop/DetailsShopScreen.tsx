...
const DetailsShopScreen = ({navigation, route}: any) => {
  ...
  const getDataDetailShop = async () => {
    try {
      const res = await authenticationAPI.HandleAuthentication(
        `/get-user-by-id?id_user=${data._id}`,
      );
  ...
  const groupProductsByServiceType = (products: ProductModel[]) => {
    ...
    products.forEach(product => {
      const serviceTypeName =
        product.id_product_type.id_service_type.service_type_name;
  ...
  const getDataProducts = async () => {
    try {
      const res = await productAPI.HandleProduct(
        `/get-product-by-id?id_user=${data._id}`,
      );
    ...
    <>
      {loading ? (
        ...
        <View
          ...
          <ImageBackground
            source={{uri: details[0].data_user.shop_banner}}
          ...
          <SectionComponent
            ...
            <RowComponent justify="center" styles={{marginVertical: 8}}>
              ...
              <TextComponent
                text={details[0].data_user.shop_name}
            ...
            <TextComponent
              text={`Địa chỉ: ${details[0].address}.`}
              color={COLORS.GRAY_WHITE}
              size={12}
            />
            <TextComponent
              text={'Giặt khô, đảm bảo trước khi giao cho khách.'}
              color={COLORS.HEX_BLACK}
              size={12}
              styles={{marginVertical: 8}}
            />
            <TextComponent
              text={'Ủi thẳng, xả cực thơm.'}
              color={COLORS.HEX_BLACK}
              size={12}
            />
            <TextComponent
              text={'99% khách hàng hài lòng về dịch vụ.'}
              color={COLORS.HEX_BLACK}
              size={12}
              styles={{marginVertical: 8}}
            />
          ...
          <SectionComponent styles={{position: 'relative', top: 120}}>
            <RowComponent justify="space-between">
              <RowComponent>
                ...
                <TextComponent
                  styles={{marginLeft: 5}}
                  text={details[0].data_user.star_rating}
                  color={COLORS.HEX_BLACK}
              ...
              <RowComponent>
                ...
                <TextComponent
                  styles={{marginLeft: 5}}
                  text={details[0].data_user.order_count}
                  color={COLORS.HEX_BLACK}
                  font={FONTFAMILY.montserrat_medium}
                  size={13}
                />
          ...
          <SectionComponent
            styles={{position: 'relative', top: 120, paddingBottom: 520}}>
            <CardProductComponent
              isCart={false}
              groupProductsByServiceType={groupProductsByServiceType(products)}
            />
          </SectionComponent>
          <View
            style={{
